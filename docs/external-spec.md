...
- Local full-text search with SQLite FTS5 (trigram, 2+ characters — terms of 3+ characters use the trigram index, a 2-character term falls back to a `LIKE` scan ordered by recency; see [db-schema.md](db-schema.md))
