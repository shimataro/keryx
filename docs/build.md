...
push and never publishes anything. AGP wires `assembleRelease`/`bundleRelease` into
`:androidApp`'s default `build` task regardless of whether the artifact is ever consumed, but
