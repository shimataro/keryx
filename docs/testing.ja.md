...
- 記事一覧自身の検索アイコンをタップすると、検索画面がキーボードを上げた状態で開き、入力欄に
...
- 結果を開いてから戻ると、クエリと結果が残ったまま検索画面に戻ること（キーボードは自動で
  再表示されない）。
- 他のフィード/タグ/フォルダーを表示している記事一覧から、その記事一覧自身の検索アイコンで検索に
  入り、そこから戻ると、**同じ記事一覧**に戻り、元のスコープが再び選択されていること
  （`homeBackAction` の `ExitSearch`。検索アイコンはスタックを進めない）。
...
- 記事一覧を下にスクロールした状態で検索アイコンから検索を開き、何も選ばずに閉じると、先頭にでは
  なく同じスクロール位置に戻ること。すべての `PaneLayout` で確認すること（この往復は
  `NarrowPaneRow` ではなく `ArticleListPane` 内部で完結するため、デスクトップの `Triple` を含む
  狭いレイアウト限定ではない）。
- 「未読のみ」を ON にした状態で未読記事を選択し（閲覧中は既読表示のまま一覧に残る）、何も選ばずに
  検索を開いて閉じる — その記事が一覧から消えずに残っており、既読表示のままであること。選択状態や
  記事詳細ペインも、検索を開く前とまったく同じ状態に戻ること。
- 検索結果内の記事（事前に選択していたものとは別の記事）を、その行から未読に戻してから検索を閉じる
  — 既読表示に戻らず、未読として表示されること。
...
- Triple: サイドバーの検索欄にフォーカスし（タップするか「検索」行から）、その状態で記事の行か
