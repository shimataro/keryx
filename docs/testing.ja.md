...
- 他のフィード/タグ/フォルダーを表示している記事一覧から、その記事一覧自身の検索アイコンで検索に
  ...
  —— フィード一覧には戻らない（`homeBackAction` の `ExitSearch`。記事一覧の検索アイコンはスタックを
  進めないのに戻る操作はペインを1段ポップしていた、という不具合の修正）。
- フィード一覧の折りたたみ検索バーから検索に入った場合は、戻るとフィード一覧に戻り、スコープも
  検索前の状態に復元されること。クエリ自体は折りたたみバーにそのまま残り、再度タップすると同じ
  結果の検索画面に戻れること。
- セッションの途中でタブレット幅の横向き（`PaneLayout.Dual`）に回転しても、読んでいた内容から
  弾き出されないこと。また、その状態の記事一覧での最初の戻る操作が黙って無視されないこと
  （`canNavigateBack` の修正）。この幅で検索に入って戻る操作をすると、スマートフォン幅と同様に
  検索スコープを抜けられること —— `PaneLayout.Dual` の深さ 1→2 は戻る操作としては通常何もしないが、
  検索中はその間も戻る矢印が有効のままであること。
...
- 記事一覧の途中から記事を開いて戻ると、同じスクロール位置に、**スクロールアニメーション無しで**
  戻ること —— ペインが実際にアンマウントされるスマートフォン幅でも、スライドする窓がペインを画面に
  残すタブレット幅の横向きでも同様。フィード一覧および検索結果の一覧についても同じであること。
...
ベルの描画位置は `NotificationBellPlacementTest.kt`、Snackbar の方針自体は
...
- フィード一覧を表示した状態でアプリを離れ、再起動する: フィード一覧のツールバーにベルがあること
  （以前は入口が全く無かったケース）。
...
- 電話幅の 設定 ▸ クラウド同期（プロバイダー接続済み）で、プロバイダー名が 1 行に収まり、末尾の
  アイコンボタン 2 つが横に並ぶこと（OS の表示サイズ最大＋アプリの文字サイズ最大でも）。未接続行の
  連携ボタンが primary 塗り、リセットが errorContainer 塗り、連携解除が枠線として、いずれも
  ティール地の上で器が判別できること。各アイコンを長押ししてツールチップを確認。リセット実行中は
  スピナーが器の上で見えること・行の高さが変わらないこと。ライト／ダーク両テーマで確認する。
