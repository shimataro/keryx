...
- `keryx.db` をダウンロードし、`feeds` テーブルの `UNIQUE(url)` 制約を緩めてから（SQLite には
  ...
  再アップロードして同期する: 同じ「同期データをリセット」通知が出ること——制約に違反するクラウド
  データも、正真正銘の破損と同様に扱われることの確認。
