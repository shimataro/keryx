...
- **DataSource layer**: Converts Ktor / SQLite exceptions into `KeryxException` subclasses. Never leaks raw exceptions upward.
  ...
  - `DatabaseMerger.merge`: classifies a merge failure from SQLite's **error code** (`SQLiteException.resultCode`, not message text) into `CloudDataIncompatibleException` (corrupt file, foreign/legacy schema, or a constraint violation the cloud DB's own — laxer — schema allowed) or leaves it unchanged (transient / an app bug). See "Merge Failure Classification" in [sync-architecture.md](sync-architecture.md).
