...
- **Not actually uncovered — noted here for contrast:** the Storage Access Framework file picker's write-failure path, Keystore-backed token storage's fallback path, and Play services' partially-granted Google Drive consent **are** covered, by `FilePickerDeviceTest.kt`, `KeystoreTokenStorageDeviceTest.kt` and `PlayServicesGoogleDriveAuthDeviceTest.kt` respectively (see `androidDeviceTest/` in "Structure" above).
