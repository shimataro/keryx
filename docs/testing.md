...
- **Not actually uncovered — noted here for contrast:** the Storage Access Framework file picker's write-failure path and Keystore-backed token storage's fallback path **are** covered, by `FilePickerDeviceTest.kt` and `KeystoreTokenStorageDeviceTest.kt` respectively (see `androidDeviceTest/` in "Structure" above).
