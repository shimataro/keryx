...
- **(Linux)** On a real X11 session (KDE Plasma, the reference environment used elsewhere in this
  section), drag a feed row and a folder header: while hovering over a valid drop position in the
  list the cursor shows the normal "move allowed" icon rather than getting stuck on the forbidden
  one (X11 AWT discards Compose's drag ghost, so these stock cursors are the only feedback there —
  see `LinuxDragCursorFix`), and dragging outside the list entirely still shows the forbidden icon.
  Repeat once on a Plasma Wayland (XWayland) session if available.
