...
- 同期ファイルは `keryx.db`（SQLite）を**アップロード前に gzip 圧縮**する。ただしライブ DB をそのまま読むのではなく、
...
2. クラウドの `keryx.db.gz` を一時ファイルへストリームしてから展開する（`MAX_SYNC_DB_SIZE_BYTES`、
   ...
   - 展開後のファイルは、マージャが開く前に SQLite の 16 バイトファイルヘッダと照合される
     ...
     なる前に、`CloudDataIncompatibleException` として即座に弾かれる。`.gz` のペイロードがそもそも
     有効な gzip でない場合（展開自体が例外を投げる場合）も、このチェックに到達する前に同様に弾かれる。
...
6. 成功したら `last_synced_at`、アップロードしたスナップショット（非圧縮）のダイジェスト、および
   **そのアップロードが生成したリビジョン**を記録する（`CloudStorage.upload` / `create` が返す）。これは
   ...
   その隙間に他デバイスがより新しい書き込みをしていると、そのリビジョンを記録してしまい、次回同期が
   **未マージの内容のダウンロードをスキップ**してしまうため。
...
（`/keryx.db`、この機能より前の全バージョンが書いていた形式）は**読み取り専用**で、圧縮ファイルが
クラウドにまだ存在しない場合（上記手順1a）にのみ参照される — このデバイスが圧縮機能追加後まだ一度も
同期していないクラウド、またはどのデバイスもまだ書き込んでいないクラウドである。`CLOUD_DB_GZ_PATH`
が一度存在すれば、レガシーファイルは二度と読まれない。またこのアプリがそれを削除・リネーム・上書き
することも一切ない — リセットもレガシーには手を触れない理由は後述の「クラウドデータのリセット
（退避）」を参照。

実DBでの実測（3,671記事・21.4MB のスナップショット、`java.util.zip.GZIPOutputStream` のデフォルト
レベル）: **アップロードは 3.47MB、84%削減**。圧縮時間は約0.3秒。`platform/Gzip` は双方向とも
ファイル間でストリームする（上記「DB をメモリに載せない」と同じ規律）ため、これによってペイロード
全体をメモリに載せる挙動が再度持ち込まれることはない。

アップロードスキップ用のダイジェスト（手順5）は意図的に**非圧縮**のスナップショットに対して計算し、
...
- **`CloudSession` 内でのトークンのライフサイクル。** リフレッシュトークンが無いため、通常の
  ...
  「不明」と認めるより悪いからである。
- **再同意。** ユーザーが Google アカウント側で付与を取り消すと、トークン供給は `null` を返し、
  `withCloudToken` がそれを `CloudAuthException` に変換し、既存の同期エラー経路が
  `ShowSettingsTab("cloud_sync")` 付きで通知センターに記録する — Dropbox のリフレッシュトークンが
  失効したときとまったく同じ扱いである。
...
`SyncRepository.resetCloudData()` は、アプリが使用できないクラウドDBからの復旧手段である。クラウド上のファイルをいきなり削除はしない — `archiveCloudDb()` がまず `CloudStorage.rename()` でタイムスタンプ付きパスへリネームする（`core/CloudBackupPath.kt` の `cloudBackupPath(clock.nowMillis())`、例: `/keryx-20260811-103000.db.gz.bak`。端末のタイムゾーンに関わらず同じ瞬間なら同じ名前になるよう UTC で整形）。その後 `createFresh()` がローカルDBのスナップショットを新しい `/keryx.db.gz` として再アップロードする。退避ファイルは自動削除されない — `CloudStorage` に一覧取得APIが無いことに加え、この仕組みを取り除くと「誤判定によるリセット」や「破損に見えただけのマージバグ」からの復旧手段自体が失われてしまう。

- **リセットが触れるのは `CLOUD_DB_GZ_PATH` のみ**である。レガシーの `CLOUD_DB_PATH` フォールバックは存在していても意図的に一切手を触れない — ロールバック経路として使えるままにしておくため。これにより、移行の途中で（レガシーファイルの破損が真因の）マージ失敗が起きた場合でもリセットが自己修復として機能する（上記「圧縮アップロード / レガシーフォールバック」参照）: ここで作られる新しく有効な `.gz` が以降のあらゆる同期で参照される正本になり、破損したレガシーファイルは二度と参照されない。
...
- **削除へのフォールバック**: リネーム自体がストレージ側の理由（退避先が既に存在する等）で失敗した場合、`archiveCloudDb()` は `cloud.delete(CLOUD_DB_GZ_PATH)` にフォールバックし、リセットが恒久的に行き詰まることを防ぐ。`CloudAuthException` だけは削除へリトライしない — 同じ資格情報の欠如で削除も同様に失敗するため。
- **退避ファイル名**: あえて `CLOUD_DB_GZ_PATH` から派生させていない（`core/Constants.kt` の `CLOUD_DB_BACKUP_PREFIX`/`CLOUD_DB_BACKUP_SUFFIX`）。退避ファイルのベース名が Google Drive の `name = 'keryx.db.gz'` 検索や OneDrive のベース名アドレッシングに一致してしまうと、`CloudStorage.metadata(CLOUD_DB_GZ_PATH)` が退避ファイルまで拾ってしまうため。
