...
どの幅でも、検索は独立した行き先ではなく、いま選択されている購読リストの項目を絞り込むものである。
3ペイン幅ではこの選択はその間ずっとサイドバーに表示・ハイライトされたままになり、クエリが何に対して
照合されているかを常に明示する。それより狭い幅ではフィード一覧はドロワーであり、検索に辿り着く
時点（記事一覧自身のヘッダーから）で通常すでに閉じているため、検索中は選択が画面に見えていない。
いずれの場合も、クエリを消す（狭い幅では展開された検索欄から戻る）と、その同じ選択の絞り込み前の
リストにそのまま戻る。
