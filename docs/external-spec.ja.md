...
- **現在のスクロール位置より一覧の新しい側の端寄りに届いた新着記事は、気づかれないまま静かに
  積み重なるのではなく、フローティングのピル**（例：「新着 12 件」）**として提示される**——
  タップするとその新着記事へ直接ジャンプする。新しい側の端は並び順に従い（新しい順なら上端、
  古い順なら下端）、その端までスクロールすればピルは必ず消える。一覧は公開日時順に並ぶため、
  新着記事が一覧の途中や、表示範囲より古い側（公開日時が古い、または無い記事）に入ることもある。
  そうした記事は、新しい側の端へスクロールしても画面に入らないため数えない。一覧を開いたまま新着が発生するケース（手動更新／バックグラウンド更新、
  ...
  既に表示されているものだからである。また、既存の記事が一覧に入り直しただけの場合
  （スターを外して付け直した、表示中のフォルダー／タグにそのフィードを移した、など）も新着とは
  数えない——新たに届いた記事ではないため。
- スター（永続）、外部ブラウザーで開く
