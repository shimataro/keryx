...
自身の Pull to refresh インジケーターを使う）。全フィードを更新するツールバーの「すべて更新」は
...
フィードが一件もないときは無効になる。「すべて更新」と同じく、クラウドに接続していれば更新のあとに
同期が続き、インジケーターはフィードの取得だけでなくその同期が終わるまで表示される。更新や同期が
すでに動いている間（バックグラウンド更新など）に引っ張った場合は新たには何も始めず、動いている
処理が終わるまでインジケーターを表示し続ける。
