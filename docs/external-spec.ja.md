...
ならないため、常に3ペインすべてを、フィード一覧を常設のサイドバーとして表示する。それより狭い幅では、
...
持たない記事一覧の常設隣人としてリーダーが表示されるタブレット幅の画面（どちらの向きでも、
Gmail 自身のタブレット閲覧ペインと同じ構成）——同じリーダー・同じ一覧順・同じ次/前操作が、
リーダーの隣に戻るボタンがあるかどうかに関わらず適用される。デスクトップ幅の3ペインレイアウトでは
