...
決して重ならない。記事リーダー本文は実ブラウザビューの内部でスクロールしており、上記どちらとも
異なる、そのプラットフォーム自身のスクロールインジケーターの作法に従う——デスクトップなら常時
表示のクラシックなスクロールバー、Android ならフェードするオーバーレイである。いずれの場合も
OS ではなくアプリ内のライト/ダーク設定に従い、それは自身が描くスクロールバーやフォーム
コントロールの配色にも及ぶ。
