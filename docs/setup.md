...
- **(Linux) After installing the `.deb`/`.rpm`, Keryx never appeared in the GNOME/KDE application menu (fixed)**: caused by `nativeDistributions.linux { }` in `composeApp/build.gradle.kts` not setting `shortcut = true` — jpackage's Linux bundlers only install a `.desktop` entry under `/usr/share/applications/` (the location GNOME Shell's and KDE Plasma's app menus actually scan) when a shortcut is explicitly requested. Without it, confirmed on real hardware, the only `.desktop` file jpackage produces stays inside the app's own install directory (`/opt/keryx/lib`), which neither launcher ever scans — no amount of logging out/in or rebuilding menu caches would have surfaced it, since the file was never in a scanned location to begin with. Fixed by adding `shortcut = true`. If a build predating this fix is still installed, remove it first (`sudo dpkg -r keryx` or the rpm equivalent) before installing a current package, since a stray `/opt/keryx` from the old install could otherwise linger alongside the new one.
