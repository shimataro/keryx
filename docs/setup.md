...
- **Android release signing keystore (optional)**: **Without a keystore, the root `./gradlew build`
  ...
  `.key.alias`/`.key.password`) signs only the AAB submitted to Google Play with a dedicated
  upload key — leaving it unset simply signs that AAB with the same keystore as everything else,
  so the four keys above are enough to build and install everything locally; see build.md's
  "Publishing to Google Play" for why the two keys exist in the first place.
