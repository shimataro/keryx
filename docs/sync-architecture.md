...
How the scheme is registered with the OS differs per platform. macOS declares it in Info.plist (`CFBundleURLTypes`) at packaging time. Windows and Linux register it at startup, from `registerCustomUriScheme()`: the Windows path writes `HKEY_CLASSES_ROOT\keryx`, the Linux path writes a user-level `.desktop` entry (`~/.local/share/applications/keryx-url-handler.desktop`) and a `~/.config/mimeapps.list` association via `LinuxUriSchemeRegistrar`. The Linux entry's `Exec` line must end in `%u` — without it the desktop-entry spec does not hand the URI to the process, and the browser cannot resolve the scheme at all (an "unknown protocol" error). On both platforms the OS then launches the app with the URL as a command-line argument, which `main.kt` forwards to the running instance via single-instance.
