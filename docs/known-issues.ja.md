...
**状態**: X11 では修正済み（`platform/LinuxDragCursorFix.kt`）。Wayland では未修正 —
XWayland／コンポジター側の制限であり、見た目だけの問題（ドロップ自体は常に成功する）。
...
カーソルアイコンの問題に対してはどちらも釣り合わない対応である。将来ネイティブ Wayland 対応の
Compose Desktop ツールキットが登場したら、まだ壊れていると決めつける前に Wayland セッションで
再確認すること。
