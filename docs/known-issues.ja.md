...
`fontScale = 1.0` における `general` タブ自身の自然な内容高さ（実測 399dp）＋若干の余裕。更新が
利用可能な状態の `updates` タブが僅差の次点で、実測は約 397dp——両方の実測値はこの定数自身の
KDoc を参照）。これで全タブの測定結果が同一になり、タブ切替でウインドウがリサイズされることが
なくなる。このアーティファクトは「表示中のウインドウをリサイズすること」と不可分であり、順序を
強制すると悪化するため、リサイズをなくすことがアプリコードから可能な唯一の対処である。
