...
| `ShowInfoDialog(detail)` | トークンの保存が平文ファイルへフォールバックしたとき（`CloudSession`）——OS のセキュアな認証情報ストアに到達できず `TokenStorage.save()` が `TokenSaveOutcome.PLAINTEXT_FILE` を返した場合 | 原因と対処法の説明ダイアログを表示（画面遷移しない） |
