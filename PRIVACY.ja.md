...
のプロファイルは、代わりにアプリ専用のプライベートキャッシュ領域に保存され、OS の
判断でストレージ逼迫時に削除されることがあります。いずれの場合も上記のアプリデータ
ディレクトリとは別の場所であり、自分で削除する方法は後述の「データの保持・削除」を
参照してください。
...
除外されています。この設定は端末の Android バックアップ設定からいつでも
...
- **認証情報の保存**: デスクトップでは、アクセストークン・リフレッシュトークンは
  ...
  利用できない場合は、アクセス権限を制限した（`0600`、自分のアカウントからのみ読める）
  ローカルファイルへフォールバックします。Android では、Android
...
- **各フィード自身のサイト/favicon** — フィード横に表示する小さなサイトアイコンの
  取得のため。
- **GitHub**（リリース確認は認証なし・匿名の `api.github.com`。実際にアップデートを
...
- **記事自体が参照するコンテンツ** — 記事を開くと、その本文の HTML が埋め込む外部
  ...
  リーダーがそれぞれの元のホストから直接読み込みます。Keryx はこれらの通信を
  フィルタリング・プロキシ・ブロックしません — 通信先は記事自体のコンテンツ次第
  であり、Keryx が選んで取得しているものではありません。

上記で述べた、開いた記事自体のコンテンツが追加で参照しうる通信先を除けば、
通信先の**種類**はこれがすべてです——これらの通信にアナリティクスやトラッキング目的のデータが
含まれることは一切なく、このリストに無いサーバーへ通信することもありません。
...
- ローカルデータをすべて消去するには: デスクトップでは、上記のアプリデータ
  ディレクトリから `keryx.db` と `local_settings.json` を削除し、あわせて
  「端末に保存されるデータ」で述べたキャッシュディレクトリ(favicon キャッシュと
  記事リーダーの WebView データを保持)も削除するか、アプリ自体をアンインストール
  してください。Android では、これらのファイルはユーザーが直接触れられない
  アプリ専用領域にあるため、アプリのアンインストール、または Android の設定 →
  アプリ → Keryx → ストレージ → ストレージを消去、を使ってください(これにより
  アプリのキャッシュも消去されます)。
...
- Keryx 自身の通信（クラウド同期・更新チェック・更新ダウンロード）は常に HTTPS を
  使用します。購読するフィードの URL は、スキームを明示しない場合にのみ HTTPS に
  補われます。明示的に `http://` のフィード（またはそこへリダイレクトされる
  フィード）を購読した場合、他の RSS リーダーと同様、そのフィードは平文の HTTP
  で取得されます。
