...
- **マルチデバイス同期**: クラウドストレージ経由（Dropbox / Google Drive / OneDrive — Android では
  現在 Dropbox と OneDrive に対応。Google Drive は対応予定）
...
現在 Windows / macOS / Linux / Android に対応。iOS/iPadOS への対応は今後の予定——それとは別に、
現在の macOS アプリもよりプラットフォームネイティブな作りへ将来刷新する、長期的な取り組みを予定している。
