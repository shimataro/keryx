...
/**
 * A tappable settings row — see `ui/settings/SettingsComponents.kt`'s
 * `LinkRow`/`ActionLinkRow`/`SwitchRow`. Desktop's `actual` keeps the app's flat convention
 ...
 *   label in the default (non-primary, non-underlined) text color — only the switch itself is
 *   ever the visibly-interactive element there.
