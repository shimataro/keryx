 ...
 *
 * @return `true` if [uri] was a `keryx://` callback and was dispatched, so the caller can decide
 * whether to clear the intent's data (avoiding reprocessing it on a later recreation, e.g. a
 * screen rotation replaying the same `Intent`).
 */
fun dispatchOAuthCallbackIfPresent(koin: Koin, uri: String?): Boolean {
    ...
    koin.get<MutableSharedFlow<OAuthCallbackParams>>().tryEmit(parseOAuthUri(uri))
    return true
}
