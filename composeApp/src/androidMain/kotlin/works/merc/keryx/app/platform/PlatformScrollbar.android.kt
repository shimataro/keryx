...
/**
 * [minLengthPx] (the dp-fixed floor, resolved to px once at composition time) as a fraction of a
 * track whose length is only known once this indicator is actually measured, so the absolute
 * floor is converted at draw time rather than baked in as a fixed fraction — a fixed fraction
 * would make the minimum thumb size scale with the pane's own height instead of staying a
 * constant dp size.
 */
private fun minLengthFraction(minLengthPx: Float, trackLengthPx: Float): Float =
